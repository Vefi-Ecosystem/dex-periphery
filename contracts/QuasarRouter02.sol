...
import "./interfaces/IQuasarRouter02.sol";
import "./libraries/QuasarLibrary.sol";
import "./interfaces/IWETH.sol";
import "./libraries/TransferHelper.sol";

contract QuasarRouter02 is IQuasarRouter02 {
  ...
  function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
    for (uint256 i; i < path.length - 1; i++) {
      (address input, address output) = (path[i], path[i + 1]);
      (address token0, ) = QuasarLibrary.sortTokens(input, output);
      IQuasarPair pair = IQuasarPair(QuasarLibrary.pairFor(factory, input, output));
      uint256 amountInput;
      uint256 amountOutput;
      {
        ...
        amountOutput = QuasarLibrary.getAmountOut(amountInput, reserveInput, reserveOutput);
      }
  ...
  function swapExactTokensForTokensSupportingFeeOnTransferTokens(
    ...
    TransferHelper.safeTransferFrom(path[0], msg.sender, QuasarLibrary.pairFor(factory, path[0], path[1]), amountIn);
    uint256 balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
    _swapSupportingFeeOnTransferTokens(path, to);
    require(IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin, "QuasarRouter: INSUFFICIENT_OUTPUT_AMOUNT");
  }
  ...
  function swapExactETHForTokensSupportingFeeOnTransferTokens(
    ...
    assert(IERC20(WETH).transfer(QuasarLibrary.pairFor(factory, path[0], path[1]), amountIn));
    uint256 balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
    _swapSupportingFeeOnTransferTokens(path, to);
    require(IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin, "QuasarRouter: INSUFFICIENT_OUTPUT_AMOUNT");
  }
  ...
  function swapExactTokensForETHSupportingFeeOnTransferTokens(
    ...
    require(amountOut >= amountOutMin, "QuasarRouter: INSUFFICIENT_OUTPUT_AMOUNT");
